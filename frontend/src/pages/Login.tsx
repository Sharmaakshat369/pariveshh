...
export default function Login() {
  ...
  return (
    <div className="relative min-h-screen flex items-center justify-center p-4 md:p-8 overflow-hidden">
      ...
      {/* Back to Home Link */}
      <Link to="/" className="absolute top-6 left-6 md:top-10 md:left-10 text-white/70 hover:text-white flex items-center gap-2 font-medium transition-colors z-20 group">
      ...
      >
        <div className="hidden lg:flex flex-col justify-between p-12 bg-gradient-to-br from-white/10 to-transparent border-r border-white/10">
          <div>
            ...
            <h2 className="text-4xl font-serif font-bold text-white mb-4 leading-tight drop-shadow-md">
              Welcome to <br/><span className="text-accent">PARIVESH</span>
            </h2>
        ...
        <div className="p-8 md:p-12 lg:p-16 flex flex-col justify-center bg-white/95">
          <div className="text-center lg:text-left mb-10">
            ...
            <p className="text-muted-foreground">Please sign in to your PARIVESH account</p>
          </div>
          ...
          <form onSubmit={handleSubmit(onSubmit)} className="space-y-6">
            ...
            {authError && (
              <motion.div 
                ...
                className="p-4 rounded-xl bg-destructive/10 border border-destructive/20 text-sm text-destructive font-medium text-center"
              >
                {authError}
              </motion.div>
            )}
            ...
            <button 
              ...
              className="w-full py-4 bg-primary text-primary-foreground rounded-xl font-bold shadow-soft hover:shadow-lg hover:-translate-y-0.5 transition-all flex items-center justify-center gap-2 mt-4 disabled:opacity-70 disabled:hover:translate-y-0"
            >
              <LogIn size={20} /> {isSubmitting ? 'Signing In...' : 'Sign In To Portal'}
            </button>
          </form>

          <p className="text-center text-sm text-muted-foreground mt-8">
            Don't have an account? <Link to="/register" className="text-primary font-bold hover:underline transition-all">Register Now</Link>
          </p>
        </div>
      </motion.div>
    </div>
